  ...
  <div>
    <div class="char-list-item" v-for="item in unitList" :key="item.id" :data-unitId="item.id" @click="charaBaseTo(item)">
      <div class="item-type-img">
        <img class="img-max" :src="unitTypeIcon(item)" />
      </div>
      ...
      <div class="item-name">
        ...
        <div class="item-cnn">{{ item.title }}</div>
      </div>
...
function unitHeadIcon(item: unit) {
  ...
  return `${host.assetsUrl}/img_card_thumb_1_${prefab[0]}-0${item.rarity}-${prefab[1]}-${prefab[2]}.png`;
}
</script>
